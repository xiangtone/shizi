...
<div class="main-nav" flex="cross:center dir:left box:first">
    <div>
        <nav class="breadcrumb rounded-0 mb-0" flex="cross:center">
            ...
            <a class="breadcrumb-item" href="<?= $urlManager->createUrl(['admin/card/type-list']) ?>">学生卡</a>
        ...
        <div class="form-title" style="border: 0;">
            <nav class="nav nav-tabs" id="myTab" role="tablist">
                <a class="nav-item nav-link active" id="nav-level-edit" data-toggle="tab" href="#level-edit"
                   role="tab"
                   aria-controls="level-edit" aria-selected="true">学生卡生成</a>
            </nav>
        </div>
        <div class="tab-content mt-4" id="nav-tabContent">
            <div class="tab-pane fade show active" id="level-edit" role="tabpanel" aria-labelledby="nav-level-edit">
                ...
                <form method="post" class="auto-submit-form" autocomplete="off"
                      data-return="<?= $urlManager->createUrl(['admin/card/type-list']) ?>">
                    <div class="form-body">
                        ...
                        <div class="col-5">
                            <select class="form-control" name="model[product_id]" size="4">
                                <?php foreach ($cat_list as $index => $value): ?>
                                    <option
                                        value="<?= $value['id'] ?>" <?= ($list['product_id'] == $value['id']) ? "selected" : "" ?>><?= $value['name'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-5">
                                <input readonly  unselectable="on" type="hidden"  class="form-control" name="cat_type_id" value="<?= $id ?>">

                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-2 text-right">
                                <label class="col-form-label required">前缀</label>
                            </div>
                            <div class="col-5">
                                <input class="form-control" name="model[prefix]" value="<?= $model->prefix ? $model->prefix : '' ?>">

                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-2 text-right">
                                <label class="col-form-label required">开始号码</label>
                            </div>
                            <div class="col-5">
                                <input class="form-control" name="model[start_num]" value="<?= $model->start_num ? $model->start_num : '1000' ?>">
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-2 text-right">
                                <label class="col-form-label required">结束号码</label>
                            </div>
                            <div class="col-5">
                                <input class="form-control" name="model[end_num]" value="<?= $model->end_num ? $model->end_num : '1999' ?>">
                            </div>
                        </div>
                        </div>
