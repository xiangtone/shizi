...
<div class="main-body p-3" id="app">
    <a href="<?=$urlManager->createUrl(['admin/game/redical-edit', 'video_id' => $video_id])?>" class="btn btn-primary">
        添加</a>
    <!-- <a class="btn btn-primary" href="javascript:" onClick="">预览</a> -->
        ...
        <table class="table table-bordered bg-white">
            ...
            <?php foreach ($list as $index => $value): ?>
                <tr>
                    ...
                    <td><?=$value['redical_word']?></td>
                    <td><?=$value['combine_word']?></td>
                    <td><?=$value['combine_type']?></td>
                    <td><?=$value['voice_url']?>
                    ...
                    <td>
                        ...
                        <a class="btn btn-sm btn-primary"
                           href="<?=$urlManager->createUrl(['admin/game/redical-edit', 'id' => $value['id'], 'video_id' => $video_id])?>">修改</a>

                        <a class="btn btn-sm btn-danger del" href="javascript:" data-content="是否删除？"
                           data-url="<?=$urlManager->createUrl(['admin/game/redical-del', 'id' => $value['id']])?>">删除</a>
...
<script type="text/javascript">
    ...
    function search() {
        ...
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/game/redical-list'])?>",
            dataType: "json",
            data: {
                keyword: keyword,
            },
    ...
    function openwin(){
        var openUrl = "<?php echo \Yii::$app->params['charUrl'] . '?video_id=' . $video_id; ?>";//弹出窗口的url
