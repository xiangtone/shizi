...
//        file_put_contents($saveRoot.$saveDir.$saveName,$qrcode_bg);
    ...
    public function getQrcode(){
        ...
        //第一步 压缩图片
        $imggzip = $this->resize_img($headimgurl,$saveRoot.$saveDir);
        //第二步 裁减成圆角图片
        $imgs['src'] = $this->test($imggzip,$saveRoot.$saveDir,$this->avatar_w,$this->avatar_w);
        $qrcode_info = getimagesize($qrcode);
        $imgs['qrcode'] = $this->test($qrcode,$saveRoot.$saveDir,$qrcode_info[0],$qrcode_info[1],$this->qrcode_true);
        //第三步 合并图片
